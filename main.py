...
class Lab1():
    ...
    def question1 (self):
        ...
        generated_numbers = self.generate_exp_distribution(self.__lambda_param,1000)
    ...
    def m_m_1_queue (self,avg_len:float,trans_rate:float)->None:
        ...
        for count , arrival_packets in enumerate(arrival_packets):
            ...
            if queue_idle:
                ...
                departure_list.append(arrival_packets+transmission_times[count])
            else:
                # if queue has packets: departure of packet= departure of i-1 + transmission of i
                departure_list.append(departure_list[count-1]+transmission_times[count])

        #TODO: Observer(monitor the queue)
        #number of observers = 
        observer_list=self.generate_exp_distribution(self.__lambda_param*5,5)
